...
class KyHopDongPage extends StatefulWidget {
  ...
  KyHopDongPage({required this.contract, required this.contractuser1, required this.contractuser, required this.contractparty, required this.contractchannel, required this.contractchannelotp});

  @override
  _KyHopDongPageState createState() => _KyHopDongPageState();
}

class _KyHopDongPageState extends State<KyHopDongPage> {
    ...
    try {
      ...
      final updatedContractsUser = widget.contractuser1
        ...
        if (user.contractCode == widget.contract.contractCode) {
          return user.copyWith(UserSignSatus: 'CONFIRMED');
        } else {
          return user;
        }
      })
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Ký hợp đồng', style: TextStyle(color: Colors.white)),
      ...
      floatingActionButton: FloatingActionButton(
        ...
        child: _isLoading ? CircularProgressIndicator(color: Colors.white) : Icon(Icons.edit),
      ),
    );
  }
}
